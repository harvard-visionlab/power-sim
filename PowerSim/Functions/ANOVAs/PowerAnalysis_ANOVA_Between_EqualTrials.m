function power_results = PowerAnalysis_ANOVA_Between_EqualTrials(prefs)
...
nPilotTrials = max(max(prefs.sub_trial_counts));
...
for f1 = 1:prefs.f1_num_levels
    for f2 = 1:prefs.f2_num_levels
        ...
        for s = 1:nPilotSubs(f1,f2)
            pilot_data{f1,f2}(s,:) = prefs.data(prefs.data(:,1) == sub_nums{f1,f2}(s) ,2)';
        end
    end
end
