function power_results = PowerAnalysis_ANOVA_Mixed_UnequalTrials(prefs)
...
maxTrialsNum = max(nPilotTrials(nPilotTrials>0));
for f1 = 1:num_between_levels
    for f2 = 1:num_within_levels
        for s = 1:nPilotSubs(f1)
            nTrialsInPilot{f1}(s,f2) = sum(prefs.data(:,1) == between_subs{f1}(s) & prefs.data(:,3) == f1 & prefs.data(:,4) == f2);
        end
    end
end
...
for f1 = 1:prefs.f1_num_levels
    pilot_data{f1} = nan(nPilotSubs(f1), maxTrialsNum, prefs.f2_num_levels);
    for s = 1:nPilotSubs(f1)
        for f2 = 1:prefs.f2_num_levels
            pilot_data{f1}(s,1:nTrialsInPilot{f1}(s,f2),f2) = prefs.data(prefs.data(:,1) == between_subs{f1}(s) & prefs.data(:,3) == f1 & prefs.data(:,4) == f2, 2)';
        end
    end
end
        ...
        for f1 = 1:prefs.f1_num_levels
            ...
            while size(sub_scores, 1) < total_num_subs(f1)
                ...
                for f2 = 1:prefs.f2_num_levels
                    ...
                    cond_trials_per_sub = nTrialsInPilot{f1}(sim_subs,f2);
