function power_results = PowerAnalysis_ANOVA_Within_UnequalTrials(prefs)
...
for s = 1:nPilotSubs
    ...
    for f1 = 1:prefs.f1_num_levels
        for f2 = 1:prefs.f2_num_levels
            cond = cond + 1;
            d = prefs.data(prefs.data(:,1) == s & prefs.data(:,3) == f1 & prefs.data(:,4) == f2, 2)';
            if length(d) < maxTrialsNum
