function [p,t,stats,terms] = anovan_matrix(y,group,varargin)
...
narginchk(2,Inf);
% if (numel(y)~=length(y))
%    error(message('stats:anovan:VectorRequired'));
% end
...
if ~isempty(varargin) && nargin<=6
   ...
   if ~ischar(va1) || isempty(va1)
      newstyle = false;
   elseif ismember(va1,{'l' 'i' 'f' 'linear' 'interaction' 'full'})
      newstyle = false;
   end
end
...
if newstyle
   ...
   defaults = {1       3        ''         'on'      0.05    ...
               false(ng,1)  []           []};
   [model sstype varnames displayopt alpha ...
...
sst = sum(y.^2);
if ~dostats
   [ssx,dfx,dmat2,y] = dofit_matrix(dmat, y, cmat, sst, mu);
else
   ...
   [ssx,dfx,dmat2,y,stats] = dofit_matrix(dmat,y,cmat,sst,mu);
...
sswo = repmat(ssw, 1, size(y,2));                   % sum of squares without this term
...
ssw = repmat(ssx, max(k), 1);        % for full model we already know the results
dfw(k) = dfx;
...
fitsubmodels_matrix;   % nested function XYZ left off here

% STEP 7A:  Compute expected means squares if requested (RWS: doems = false)
if doems
...
pval = NaN(size(fstat));
t = repmat((dfdenom>0 & dfterm>0), 1, size(pval,2));
pval = fpval_matrix(fstat, dfterm, dfdenom);
p = pval;
return
...
function fitsubmodels_matrix
   ...
   for j=length(sindices):-1:1
      ...
      % If this model is the "without" model for a term, compute L
      %RWS note: doems is false, so skipping this
%       if doems && j>nterms
%          if sstype==1
%             prevterms = (1:nterms)<k;
%          else
...
function [tbl,t] = maketable
   ...
   if ~isempty(randomterm)
      ...
      tbl(1,end+1:end+8) = {getString(message('stats:anovan:RowHeadType'))        getString(message('stats:anovan:RowHeadExpectedMS')) ...
                            ...
                            getString(message('stats:anovan:RowHeadDenomDefn'))   getString(message('stats:anovan:RowHeadVarEst'))     ...
...
function p = fpval_matrix(x,df1,df2)
...
xunder(isnan(x)) = NaN;
c = size(x,2);
p = fcdf(xunder,repmat(df2, 1,c),repmat(df1,1,c));
end
...
function [ssx,dfx,dmat2,y2,stats] = dofit_matrix(dmat,y,cmat,sst,mu)
%DOFIT Do constrained least squares fit and reduce data for subsequent fits
...
b = Qc0 * z;             % coefficients back in full space
ssx = vecnorm(y2).^2;        % sum of squares explained by fit
...
%   $Revision: 1.13 $  $Date: 2005/01/13 19:14:37 $
