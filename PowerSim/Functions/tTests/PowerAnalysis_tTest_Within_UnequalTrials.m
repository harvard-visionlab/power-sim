function power_results = PowerAnalysis_tTest_Within_UnequalTrials(prefs)
...
maxTrialsNum = max(max(nPilotTrials));

%organize data sub*trial*cond - pad extra trials with 0s
pilot_data = zeros(nPilotSubs, maxTrialsNum, nConds);
for s = 1:nPilotSubs
    for cond = 1:nConds
        d = prefs.data(prefs.data(:,1) == s & prefs.data(:,3) == cond, 2)';
        if length(d) < maxTrialsNum %if fewer trials than max, pad w/ nan
           d = [d, nan(1, maxTrialsNum - length(d))];
        end
        pilot_data(s,:,cond) = d;
    end
end
    ...
    for sub_count = 1:length(sub_vector)
        ...
        while size(sub_scores, 1) < total_num_subs
            ...
            for c = 1:nConds
                ...
                cond_trials_per_sub = nPilotTrials(sim_subs, c);
            ...
            %cond_scores = [cond_scores, cond_scores_tmp_all(
